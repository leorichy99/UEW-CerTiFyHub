...
import {
  ...
  ShieldCheck, Check, Loader2, FileX,
} from "lucide-react";
...
export default function CertificatePreview({ certificate, onClose }) {
  ...
  }, [onClose, zoomIn, zoomOut, toggleFullscreen]);
  ...
  const watermarkText = isRevoked ? 'REVOKED' : 'ISSUED';
  const watermarkColor = isRevoked ? 'text-rose-200' : 'text-slate-300';
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div className="flex flex-1 min-w-0 flex-col">
            ...
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 py-3.5">
              <div className="min-w-0">
                <h2 className="text-[20px] font-bold tracking-wide text-slate-900 uppercase">
                  Certificate Preview
                </h2>
            ...
            >
              {/* Diagonal Watermark Pattern - covers entire viewing stage */}
              <div className="absolute inset-0 z-10 overflow-hidden pointer-events-none">
                {[...Array(10)].map((_, i) => (
                  [...Array(10)].map((_, j) => (
                    <div
                      key={`${i}-${j}`}
                      className={`absolute font-bold sans-serif opacity-15 ${watermarkColor}`}
                      style={{
                        fontSize: '130px',
              ...
              {loading ? (
                ...
                <div className="flex min-h-full items-center justify-center p-6 sm:p-10">
                  ...
                  >
                    ...
                    <div className={`absolute -top-2.5 left-[50%] translate-x-[-50%] z-10 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider shadow-md ring-2 ${isRevoked ? 'bg-rose-50 text-rose-700 ring-1 ring-rose-100'
                    ...
                    <div className="overflow-hidden rounded-lg border border-slate-200/80 bg-white shadow-[0_10px_30px_-12px_rgba(15,23,42,0.25),0_2px_6px_rgba(15,23,42,0.05)]">
                      <img
                        ...
                        className="block max-w-[78vw] max-h-[68vh] object-contain z-50"
                        draggable={false}
                      />
                    </div>
                  </motion.div>
              ...
              {previewUrl && !loading && (
                <div className="absolute inset-x-0 bottom-4 flex justify-center z-30">
                  <div className="inline-flex items-center divide-x divide-slate-200 rounded-full border border-slate-200 bg-white/95 px-1 py-1 shadow-lg backdrop-blur">
            ...
            <div className="flex items-center gap-2 overflow-x-auto border-t border-slate-200 bg-white px-4 py-2.5 text-[11px] md:hidden">
              ...
              <span className="truncate text-slate-600">{(certificate?.student_name)?.toUpperCase() || '—'}</span>
              <span className="text-slate-300">·</span>
          ...
          <aside className="hidden md:flex w-[400px] shrink-0 flex-col border-l border-slate-200 bg-slate-50/40">
            {/* Recipient avatar card */}
            <div className="mx-4 my-6 flex flex-col items-center bg-white p-5">
              <div className={`flex h-30 w-30 items-center justify-center rounded-full text-xl font-semibold text-slate-500 bg-slate-300`}>
                {getInitials(certificate?.student_name)}
              </div>
              <div className="mt-3 flex gap-3">
                <p className="text-sm font-medium text-slate-900">{(certificate?.student_name)?.toUpperCase() || '—'}</p>
                    {/* <span
            ...
            {/* Certificate Details */}
            <div className="px-5">
              <h3 className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400 text-center">
                Certificate Details
              </h3>
